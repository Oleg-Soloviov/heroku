...
class TextInputFieldsForm(forms.Form):
    ...
    ip = forms.GenericIPAddressField(label='GenericIPAddressField', help_text='127.0.0.1', required=False)
...
#        self.helper.form_class = 'blueForms'
        ...
        self.helper.layout = Layout(
            HTML("<p>These Django fields use <strong>TextInput</strong> as a default widget, so browsers will not apply their own validation on these fields. Try to input the wrong value and submit the form - browser validation will not warn you  about an error and you will get a Django validation error.</p>"),
            'your_name',
            'my_slug',
            'my_time',
            'my_uuid',
...
#        self.helper.form_class = 'blueForms'
        ...
        self.helper.layout = Layout(
            HTML("<p>These Django fields use <strong>TextInput</strong> as a default widget, so browsers will not apply their own validation on these fields. Try to input the wrong value and submit the form - browser validation will warn you  about an error.and you will get a Django validation error.</p>"),
            'my_url',
            'my_email',
            'integer',
            'my_float',
